...
public class LevenshteinDistance {
  ...
  private static int computeDistanceBetweenPrefixes(String A, int aIdx, String B, int bIdx,
    ...
    if (distanceBetweenPrefixes[aIdx][bIdx] == -1) {
      if (A.charAt(aIdx) == B.charAt(bIdx)) {
        distanceBetweenPrefixes[aIdx][bIdx] = computeDistanceBetweenPrefixes(A, aIdx - 1, B,
            bIdx - 1, distanceBetweenPrefixes);
      } else {
        int substituteLast = computeDistanceBetweenPrefixes(A, aIdx - 1, B, bIdx - 1,
            distanceBetweenPrefixes);
        int addLast = computeDistanceBetweenPrefixes(A, aIdx, B, bIdx - 1, distanceBetweenPrefixes);
        int deleteLast = computeDistanceBetweenPrefixes(A, aIdx - 1, B, bIdx,
            distanceBetweenPrefixes);
  ...
  public static void main(String[] args) {
    System.out.println(levenshteinDistance("benyam","ephrem"));
  }
}
// Output: 5